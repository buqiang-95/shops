...
<body>
  <form class="form-horizontal" role="form" method="post" action="{{url('/fen/store')}}">
    @csrf
  <div class="form-group">
    <label for="firstname" class="col-sm-2 control-label">分类名称</label>
    <div class="col-sm-10">
      <input type="text" name="f_name" id="firstname" placeholder="请输入分类名称">
    </div>
  </div>
  <div class="form-group">
    ...
    <div class="col-sm-10">
      <select name="parent_id">
          ...
          @foreach($data as $v)
          <option value="{{$v->f_id}}">{{'|'.str_repeat('|—',$v->level)}}{{$v->f_name}}</option>
          @endforeach
      </select>
    </div>
  </div>
